...
// Three leaderboard variants:
//
//  1. <TestLeaderboard testId={1} />
//     — compact panel shown on the TestPage sidebar
//     — refreshes every 60s while test is active
//
//  2. <ResultLeaderboard testId={1} testTitle="..." userElapsed={924} />
//     — full panel shown on the right of the ResultPage
//     — shows top 10 + user's personal rank box
//
//  3. <HomeLeaderboard />
//     — top 5 on the HomePage sidebar
//     — ranked by highest avg band over most tests
...
// ─────────────────────────────────────────────────────────
//  SHARED ROW — used by both TestLeaderboard & ResultLeaderboard
// ─────────────────────────────────────────────────────────
function TestLbRow({ entry, index, isMe, showTime = true }) {
  ...
  return (
    <div style={{
      ...
      {showTime && (
        <div style={{
          textAlign: 'right', flexShrink: 0, minWidth: 38,
...
// ─────────────────────────────────────────────────────────
//  1. TEST PAGE LEADERBOARD
//     Compact — shown on TestPage right sidebar
//     Refreshes every 60s
// ─────────────────────────────────────────────────────────
export function TestLeaderboard({ testId, testTitle }) {
  ...
  async function load() {
    ...
    setLoading(true)
    try {
      const lb = await fetchTestLeaderboard(testId, 5) // only top 5 on test page
      setEntries(lb)
    ...
    <div style={{
      ...
      <div style={{ marginBottom: 10 }}>
        ...
        <div style={{ fontSize: 10.5, color: '#64748b' }}>
          Tied scores ranked by fastest time
        </div>
      </div>
...
// ─────────────────────────────────────────────────────────
//  2. RESULT PAGE LEADERBOARD
//     Full top 10 + user rank box
//     Shown on right side of ResultPage
// ─────────────────────────────────────────────────────────
export function ResultLeaderboard({ testId, testTitle, userElapsed }) {
  const { user }                    = useAuth()
  const [entries,   setEntries]     = useState([])
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([lb, ue]) => { setEntries(lb); setUserEntry(ue) })
      .catch(() => {})
  ...
  return (
    <div style={{
      ...
      {/* Column labels */}
      <div style={{
        display: 'flex', gap: 7, padding: '0 9px 7px',
      ...
      <div style={{
        marginTop: 10, paddingTop: 9, borderTop: '1px solid #f1f5f9',
        fontSize: 9.5, color: '#94a3b8', textAlign: 'center',
      }}>
        Score ties broken by fastest time · Updates after each attempt
      </div>
    </div>
  )
}
...
// ─────────────────────────────────────────────────────────
//  3. HOME PAGE LEADERBOARD
//     Top 5 real users
//     Ranked by: highest avg band × tests completed
//     (rewards consistent high performance across many tests)
// ─────────────────────────────────────────────────────────
export function HomeLeaderboard() {
  ...
  return (
    <div style={{
            ...
            {entries.map((e, i) => {
              ...
              return (
                <div key={e.userId || i} style={{
                  ...
                  background: isMe
                    ? '#eff4ff'
                    : e.rank === 1 ? 'linear-gradient(135deg,#fffbeb,#fef3c7)'
                    : '#f8fafc',
                  border: `1px solid ${isMe ? '#93c5fd' : e.rank === 1 ? '#fde68a' : 'transparent'}`,
                }}>
                  <span style={{ fontSize: e.rank <= 3 ? 13 : 11, fontWeight: 700, minWidth: 20, textAlign: 'center', color }}>
                    {icon}
                  </span>
      ...
      <div style={{
        borderTop: '1px solid #f1f5f9', marginTop: 10, paddingTop: 9,
